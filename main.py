...
def main():
    # entradas da equação
    sacoDeDormir = mochilaItens('Saco de dormir', 15, 15)
    corda = mochilaItens('Corda', 3, 10)
    canivete = mochilaItens('Canivete', 2, 10)
    tocha = mochilaItens('Tocha', 5, 5)
    garrafa = mochilaItens('Garrafa', 9, 8)
    comida = mochilaItens('Comida', 20, 17) 

    itensMochila = [sacoDeDormir, corda, canivete, tocha, garrafa, comida]
    ...
    for generation in range(num_generations):
        ...
        fitness = ga.cal_pop_fitness(itensMochila, new_population)
        ...
        best_result = np.max(np.sum(new_population*itensMochila, axis=1))
